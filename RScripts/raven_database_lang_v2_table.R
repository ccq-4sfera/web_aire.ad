...
update_raven <- function(){
  ...
  #TODO check add old data from observations
  ...
  # df <- df_obs %>% full_join(table5,by=c("sampling_point_id",
  ...
  #TODO? si fa mes de 3h que no rebem dades,
  ...
  if(FALSE){
    ...
    #TODO mail when script returns an error
  }
...
if("try-error" %in% class(t)){
  ...
  #overwrite table
  write.csv(table_out, file = "/var/rprojects/Projects/AireAD/table_errors.csv")
